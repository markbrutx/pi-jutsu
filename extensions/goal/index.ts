...
interface StatusContext {
	...
	ui: {
		...
		theme: { fg(color: "accent" | "success" | "warning" | "error" | "muted" | "dim", text: string): string };
	};
	isIdle?: () => boolean;
...
const goalCompleteTool = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		ctx.ui.setStatus(STATUS_KEY, completedGoal ? formatStatus(activeGoal, ctx) : undefined);
		clearActiveGoal(ctx);
...
function updateStatus(ctx: StatusContext, goal: ActiveGoal) {
	clearCompletionStatusTimer();
	ctx.ui.setStatus(STATUS_KEY, formatStatus(goal, ctx));
	scheduleStatusTick(ctx);
}

/**
 * Self-scheduling tick: fires exactly when the visible duration string changes
 * (every 1s under a minute, every 60s after). Each setStatus triggers a full
 * TUI re-render, so we never tick more often than the display can change.
 * Budget goals show tokens, not time, so they need no ticking at all.
 */
function scheduleStatusTick(ctx: StatusContext) {
	stopStatusTicker();
	if (!activeGoal || activeGoal.status !== "active") return;
	if (activeGoal.tokenBudget !== undefined) return;
	const elapsedMs = Date.now() - activeGoal.startedAt;
	const stepMs = elapsedMs < 60_000 ? 1_000 : 60_000;
	const delay = stepMs - (elapsedMs % stepMs) + 20;
	statusTicker = setTimeout(() => {
		...
		try {
			ctx.ui.setStatus(STATUS_KEY, formatStatus(activeGoal, ctx));
		} catch {
			// The captured ctx may be stale after session replacement; stop ticking.
			return;
		}
		scheduleStatusTick(ctx);
	}, delay);
...
function formatStatus(goal: ActiveGoal | undefined, ctx: StatusContext) {
	if (!goal) return undefined;
	const t = ctx.ui.theme;
	if (goal.status === "complete") return t.fg("success", "◎ ✓ complete");
	if (goal.status === "paused") return t.fg("warning", "◎ ❚❚ paused");
	if (goal.status === "budget_limited") return t.fg("error", `◎ ◑ budget ${formatBudget(goal)}`);
	if (goal.tokenBudget !== undefined) return t.fg("accent", `◎ » active ${formatBudget(goal)}`);
	return t.fg("accent", `◎ » active ${formatDuration(goal.timeUsedSeconds)}`);
}
...
function showCompletionStatus(ctx: StatusContext) {
	clearCompletionStatusTimer();
	ctx.ui.setStatus(STATUS_KEY, ctx.ui.theme.fg("success", "◎ ✓ complete"));
